...
We employed a grid search approach to tune the strategy’s hyperparameters for each optimizer (PSO, PSO-SA, ABC). Over 100 iterations, we explored combinations of values for parameters such as population size, acceleration coefficients, and others. Each candidate configuration was evaluated using a custom scoring function, which was designed to reflect desirable characteristics in a trading bot’s behavior. 
The final selected hyperparameters for each optimizer are listed in the table below. The number 42 was used as the seed in the grid search, as well as for initial training and testing.  
...
\begin{table}[ht]
    ...
    \begin{tabular}{llr}
      \toprule
      \textbf{Optimizer} & \textbf{Hyper-parameter}                          & \textbf{Value} \\ 
      \midrule
      \multirow{3}{*}{PSO}      
      ...
      \bottomrule
    \end{tabular}
    \caption{Optimizers and discovered optimal hyperparameters}
\end{table}
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.8\textwidth]{./assets/parameter_weights.png}
...
We inspected correlations among the ten learned hyperparameters (averaged over seeds).
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.8\textwidth]{./assets/sensitivity.png}
